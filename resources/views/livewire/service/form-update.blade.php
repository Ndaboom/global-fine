<div>
    <form class="" wire:submit.prevent="store">
        <p>{{ __('Update extra service') }}</p>
        <p class="form-row form-row-first col-lg-12">
            <input type="text" class="input-text" placeholder="{{ __('Name') }}" wire:model="extraService.description">
            @error('extraService.description') <span class="error">{{ $message }}</span> @enderror
        </p>
