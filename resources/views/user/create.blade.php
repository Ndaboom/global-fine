...
{{ Form::open(['route' => 'users.store', 'method' => 'POST', 'class' => 'max-w-7xl mx-auto py-10 sm:px-6 lg:px-8']) }}
<div class="md:grid md:grid-cols-3 md:gap-6">
    <div class="md:col-span-1">
        <div class="px-4 sm:px-0">
            ...
            <p class="mt-1 text-sm text-gray-600">
                Create Your New User And His Credentials.
            </p>
        </div>
    </div>
    <div class="px-4 py-5 bg-white border-left sm:p-6 sm:rounded-tl-md sm:rounded-tr-md md:col-span-1">
        ...
        <div class="col-span-6 sm:col-span-4 uk-margin-bottom">
            ...
            {{ Form::select('roles', $roles, null, ['class' => 'border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full']) }}
        </div>
    </div>
    ...
    <div class="flex items-center md:col-span-3 justify-end px-4 py-3 bg-gray-50 text-right sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
        <button type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">{{ __('Save User') }}</button>
    </div>
</div>
{{ Form::close() }}
@push('styles')
